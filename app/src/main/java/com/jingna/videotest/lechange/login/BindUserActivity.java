...
//		mNoticeText.setText(this.getString(R.string.bind_user_notice) + mPhoneNumber);
		...
		//默认自动填写前一个界面用户输入的账号
		mAuthCodeEdit.setText(mPhoneNumber);
	}
	...
	{
		mBindBtn.setOnClickListener(new OnClickListener() {	//获取到验证码后，点击进行绑定
			@Override
			public void onClick(View arg0) {
				checkSms();				
			}
		});
//		mGetAuthCodeBtn.setOnClickListener(new OnClickListener() {	//点击获取验证码
//			@Override
//			public void onClick(View arg0) {
//                //还在倒计时
//				if(mTime > 0)
	...
	public void checkSms(){
		...
		Business.getInstance().checkUserSms(mPhoneNumber, smsCode, new Handler(){
			...
			public void handleMessage(Message msg) {
				if(0 == msg.what)
				{
					Toast.makeText(getApplicationContext(), R.string.toast_userbind_success, Toast.LENGTH_SHORT).show();
					finish();
				}
				else
				{
