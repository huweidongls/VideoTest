package com.jingna.videotest.lechange.listview;
...
import com.jingna.videotest.lechange.business.entity.ChannelInfo;
import com.jingna.videotest.lechange.business.util.ImageHelper;
import com.jingna.videotest.lechange.common.CommonTitle;
import com.jingna.videotest.lechange.common.ProgressDialog;
import com.jingna.videotest.lechange.manager.DeviceConfigWifiActivity;
import com.jingna.videotest.lechange.manager.DeviceSetActivity;
import com.jingna.videotest.lechange.mediaplay.MediaPlayActivity;
import com.jingna.videotest.lechange.message.AlarmMessageActivity;
import com.jingna.videotest.lechange.view.XListView;
...
public class DevicelistActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mCommonTitle.setOnTitleClickListener(new CommonTitle.OnTitleClickListener() {
			...
			public void onCommonTitleClick(int id) {
				...
				case CommonTitle.ID_RIGHT:
					Intent intent = new Intent(DevicelistActivity.this, DeviceConfigWifiActivity.class);
					Logger.d(tag, "DeviceConfigWifiActivity");
		...
		//绘制list
		mListview = (XListView) this.findViewById(R.id.list_devices);
		...
		mListview.setXListViewListener(new XListView.IXListViewListener() {
			...
			public void onLoadMore() {
				String queryRange=String.valueOf(currentPosition+1)+"-"+String.valueOf(currentPosition+loadCount);
				currentPosition=currentPosition+loadCount;
				Business.getInstance().getChannelList(queryRange,new Handler()	{
					...
					public void handleMessage(Message msg) {
						...
						if (msg.what == 0) {
							if(((List<ChannelInfo>) retObject.resp).size()==0){
								currentPosition=currentPosition-loadCount;
								Toast.makeText(DevicelistActivity.this, R.string.toast_device_no_devices, Toast.LENGTH_LONG).show();
							}
							for (int i = 0; i < ((List<ChannelInfo>) retObject.resp).size(); i++) {
//								if (((List<ChannelInfo>) retObject.resp).get(i).getDeviceCode().equals("3B02A8AGAK00008") || ((List<ChannelInfo>) retObject.resp).get(i).getDeviceCode().equals("3G00A11PAA00033") || ((List<ChannelInfo>) retObject.resp).get(i).getDeviceCode().equals("3J07937PAA00245")) {
									mChannelInfoList.add(((List<ChannelInfo>) retObject.resp).get(i));
//								}
							}
	...
	private class ChannelAdapter extends BaseAdapter
	{
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null){
				...
				holder.mLiveVideo = (ImageView)convertView.findViewById(R.id.list_device_livevideo);//实时间预览
				...
				holder.mLiveVideo.setOnClickListener(new OnClickListener() {				
					...
					public void onClick(View arg0) {			
						...
						if (((holder.mInfo.getAbility() & ChannelInfo.Ability.TCM) == 0) && holder.mInfo.getEncryptMode() == 1 && holder.mInfo.getEncryptKey() == null) {
				...
				holder.mSetting.setOnClickListener(new OnClickListener(){
					...
					public void onClick(View arg0) {
						...
						DevicelistActivity.this.startActivity(intent);
						Logger.d(tag, " DeviceCode: "+holder.mInfo.getDeviceCode());
					}
				});
				//屏蔽监听
...
//			holder.mChannelName.setText(holder.mInfo.getDeviceCode() + "-" + holder.mInfo.getIndex());
			if (holder.mInfo.getEncryptMode() == 1 && ((holder.mInfo.getAbility() & ChannelInfo.Ability.TCM) == 0)) {
			...
			Logger.d(tag, " holder.mInfo.getBackgroudImgURL() : " + holder.mInfo.getBackgroudImgURL());
			...
			//是否自定义加密,效率不高
			if (holder.mInfo.getEncryptMode() == 1 && holder.mInfo.getEncryptKey() == null && ((holder.mInfo.getAbility() & ChannelInfo.Ability.TCM) == 0)) {
	...
	static class ViewHolder{
		...
		ImageView 	    mLiveVideo;//实时预览
		ImageView	    mLocalVideo;//本地录像
		ImageView       mCloudVideo;//云端录像
		ImageView       mMessage;//报警消息
		ImageView       mSetting;//设备操作
