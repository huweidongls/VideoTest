package com.jingna.videotest.lechange.business.util;
        ...
        TaskPoolHelper.addTask(new TaskPoolHelper.RunnableTask(imageID) {
            ...
            public void run() {
                ...
                try {
                    ...
                    dstLen[0] = 500000;
                    Bitmap bitmap; //=null
//					int res = LCOpenSDK_Utils.decryptPic(srcBuf,length,deviceId,key,dstBuf,dstLen);
                    //调用图片解密新接口：三码合一
                    int res = LCOpenSDK_Utils.decryptPic(Business.getInstance().getToken(), srcBuf, length, deviceId, key, dstBuf, dstLen);
